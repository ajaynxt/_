    ...
    <article class="website-editor" data-website-editor="${escapeHtml(item.id)}">
      ...
      <div class="website-editor-body">
        ...
        <label class="toggle-row review-publish">
          <span><strong>Published</strong><small>Show this website in the public portfolio.</small></span>
...
async function loadContent() {
  ...
  if (delay) delay.value = Math.max(5, Number(content.settings?.paletteDelayMs || 180000) / 1000);
}
...
document.querySelector('[data-save-settings]')?.addEventListener('click', async () => {
  try {
    ...
    next.settings = {
      ...
      paletteDelayMs: Math.max(180, Number(document.querySelector('[data-setting-delay]')?.value || 180)) * 1000,
