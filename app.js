(() => {
  ...
  const palettes = [
    {name:'lime',accent:'#c9ff38',ink:'#79a400',soft:'rgba(201,255,56,.16)'},
    {name:'gold',accent:'#e9bd72',ink:'#976116',soft:'rgba(233,189,114,.16)'},
    {name:'azure',accent:'#77b9ff',ink:'#2871bd',soft:'rgba(119,185,255,.16)'},
    {name:'coral',accent:'#ff8d72',ink:'#ba412b',soft:'rgba(255,141,114,.16)'},
    {name:'violet',accent:'#bda3ff',ink:'#6d4fc1',soft:'rgba(189,163,255,.16)'},
    {name:'emerald',accent:'#64dca4',ink:'#20805a',soft:'rgba(100,220,164,.16)'},
    {name:'rose',accent:'#ffa7c5',ink:'#b64970',soft:'rgba(255,167,197,.16)'}
  ];
  const fonts = {
  ...
  function applyAppearance(){
    ...
    let paletteIndex = Math.max(0,palettes.findIndex(p=>p.name===paletteName || (paletteName==='ember'&&p.name==='lime')));
    applyPalette(paletteIndex);
    const delay=Math.max(60000,Number(content.settings?.paletteDelayMs||60000));
    if(content.settings?.paletteRotationEnabled!==false){ paletteTimer=setInterval(()=>{paletteIndex=(paletteIndex+1)%palettes.length;applyPalette(paletteIndex);},delay); }
    if(content.settings?.headingRotationEnabled!==false){ fontTimer=setInterval(()=>{root.classList.add('font-shifting');setTimeout(()=>{fontIndex=(fontIndex+1)%fontKeys.length;root.style.setProperty('--display',fonts[fontKeys[fontIndex]]);root.classList.remove('font-shifting');},900);},60000); }
  }
