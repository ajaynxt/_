(() => {
  ...
  // Subtle 3D tilt on the hero portrait, following the cursor.
  const heroStage = document.querySelector('.hero-stage');
  const portraitFrame = document.querySelector('.portrait-frame');
  if (heroStage && portraitFrame && !reduceMotion && window.matchMedia('(hover: hover)').matches) {
    ...
    heroStage.addEventListener('mousemove', (event) => {
      ...
      frameRequest = requestAnimationFrame(() => {
        portraitFrame.style.transform = `rotateY(${px * 10}deg) rotateX(${py * -10}deg) translateZ(0)`;
      });
    });
    heroStage.addEventListener('mouseleave', () => {
      portraitFrame.style.transform = 'rotateY(0deg) rotateX(0deg)';
    });
  }
