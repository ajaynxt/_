(() => {
  ...
  const normalizeSites = source => (source.websites||[]).filter(i=>i&&i.published!==false&&i.title&&i.url).map((item,index)=>({...item,id:String(item.id||`site-${index}`),url:safeUrl(item.url),image:safeUrl(item.image)||'./assets/ajay-nxt-orange-mark.png',tags:Array.isArray(item.tags)?item.tags.slice(0,7):[],category:inferCategory(item),styleCount:inferStyleCount(item),type:isClient(item)?'client':'demo',priority:Number(item.priority||index+1)}));
  ...
  function renderDemos(sites){
    ...
    const count=items.reduce((sum,i)=>sum+i.styleCount,0); document.querySelector('[data-demo-count]')?.replaceChildren(document.createTextNode(count>=20?`${count}+`:String(count)));
    const cats=['All',...new Set(items.map(i=>i.category))];
    filters.innerHTML=cats.map((c,i)=>`<button type="button" class="${i===0?'is-active':''}" data-demo-filter="${esc(c)}">${esc(c)}</button>`).join('');
    grid.innerHTML=items.map(item=>`<article class="demo-card reveal" data-demo-category="${esc(item.category)}"><figure><img src="${esc(item.image)}" alt="${esc(item.imageAlt||item.title+' demo preview')}" loading="lazy" decoding="async"/></figure><div class="demo-card-body"><div class="demo-meta"><span>${esc(item.category)}</span><b>${item.styleCount>1?`${item.styleCount} directions`:'Live demo'}</b></div><h3>${esc(item.title)}</h3><p>${esc(item.summary||'')}</p><div class="project-tags">${item.tags.map(t=>`<span>${esc(t)}</span>`).join('')}</div><a class="text-link" href="${esc(item.url)}" target="_blank" rel="noreferrer" data-track="project_live_click">Open live demo ↗</a></div></article>`).join('');
